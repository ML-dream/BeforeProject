...
public class MyBigNodeManager extends NodeManager {
	...
	public class MyBigIoManager extends IoManager {
		...
		protected void readValue(ServiceContext serviceContext,
				Object operationContext, NodeId nodeId, UaVariable node,
				NumericRange indexRange, TimestampsToReturn timestampsToReturn,
				DateTime minTimestamp, DataValue dataValue)
				throws StatusException {
			DataItem dataItem = getDataItem(nodeId);
			if (dataItem == null)
	...
	public boolean hasNode(NodeId nodeId) {
		return nodeId.getValue().equals("MyBigNodeManager")
				|| nodeId.equals(DataItemType) || (getDataItem(nodeId) != null);
	}
	...
	protected void afterCreateMonitoredDataItem(ServiceContext serviceContext,
		...
		c.add(item);
		logger.debug("afterCreateMonitoredDataItem: nodeId={} c.size()={}",
				item.getNodeId(), c.size());
	}
