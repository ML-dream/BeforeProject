...
public class SampleConsoleClient {
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			//如果未定义消息，则命令行为空，提示时用户没有输入任何URL。否则，异常将用于通知无效的参数。
	...
	protected MonitoredDataItemListener dataChangeListener = new MyMonitoredDataItemListener(
			this);
	...
	protected final QualifiedName[] eventFieldNames = {
			...
			null, null };
	protected final MonitoredEventItemListener eventListener = new MyMonitoredEventItemListener(
	...
	protected NodeId browse(NodeId nodeId, NodeId prevId)
		...
		// 获取用于浏览的引用：首选前一个节点，但否则接受任何层次逆引用
	...
	/**
	 * 调用对象上的方法。注意，对象上调用方法，因此需要为对象和方法定义NoDID。
	 *
	 * @param nodeId
	...
	protected void createMonitoredItem(Subscription sub, NodeId nodeId,
		...
		if (!sub.hasItem(nodeId, attributeId)) {
			...
			else if (Math.abs(requestedSamplingInterval
					- revisitedSamplingInterval) > 0.01)
				println(String
						.format("The requested sampling inteval is different than reguested, requested: %s, got: %s",
								requestedSamplingInterval,
								revisitedSamplingInterval));
	...
	protected void initialize(String[] args) throws URISyntaxException,
		...
		// ...并用自定义处理程序对验证结果作出反应（如果需要的话，提示用户做什么）
		...
		appDescription.setApplicationName(new LocalizedText(APP_NAME,
				Locale.ENGLISH));
		// URI中的“本地主机”（所有小写）被转换为应用程序运行的计算机的实际主机名。
		appDescription
