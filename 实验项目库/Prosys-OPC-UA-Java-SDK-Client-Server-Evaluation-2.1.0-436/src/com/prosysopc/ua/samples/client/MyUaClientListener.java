...
public class MyUaClientListener implements UaClientListener {
	 ...
	 */
	@Override
	public void onAfterCreateSessionChannel(UaClient client, Session session)
			throws ConnectException {
		Session s = client.getSession();
		if (s.getSessionTimeout() <= UNREALISTIC_SHORT_TIMEOUT) {
			pl("The RevisedSessionTimeout is unrealistically short: "
					+ s.getSessionTimeout()
		...
		if (s.getSessionTimeout() >= UNREALISTIC_LONG_TIMEOUT) {
			pl("The RevisedSessionTimeout is unrealistically long: "
					+ s.getSessionTimeout()
					+ " ms. Do you still want to connect? y=Yes, anything else is No");
			String input = SampleConsoleClient.readInput(false).toLowerCase();
			if (!input.equals("y"))
	 ...
	 */
	@Override
	public void onBeforePublishRequest(UaClient client,
	 ...
	 */
	@Override
	public boolean validatePublishResponse(UaClient client,
			PublishResponse response) {
		return validatePublishTime(response.getNotificationMessage()
				.getPublishTime());
	}

	@Override
	public boolean validateRepublishResponse(UaClient client,
	...
	private boolean validatePublishTime(DateTime publishTime) {
		 ...
		 */
		long diff = Math.abs(DateTime.currentTime().getTimeInMillis()
				- publishTime.getTimeInMillis());
		if ((diff > ALLOWED_PUBLISHTIME_DIFFERENCE)
				&& !publishTime.equals(DateTime.MIN_VALUE)
				&& !publishTime.equals(DateTime.MAX_VALUE)) {
			pl(String
					.format("PublishResponse PublishTime difference to "
